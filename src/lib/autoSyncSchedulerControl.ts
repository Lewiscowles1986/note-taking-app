/**
 * Auto-sync scheduler control (multi-server) — the imperative half of
 * src/lib/autoSyncScheduler.tsx, split out so the component file keeps only
 * its default-export component (fast-refresh requires it).
 *
 * Reads the per-server settings synchronously (a few bytes of localStorage
 * each) and ONLY for servers with auto-sync enabled does it dynamically
 * import the sync engine and start a background interval — ONE interval per
 * configured server, each on its own cadence. Never-configured users never
 * load the engine; auto-sync users load it lazily after first paint.
 *
...
/** Stop every scheduler interval without queueing a restart (unmount path). */
