...
| **#45** | `typescript` → v7 | `test`/`lint`/`build` (all nodes), `Coverage & mutation` | Lockfile not regenerated. `package.json` bumped to `^7.0.0` but `package-lock.json` still pins `typescript@5.8.3`, so **every** `npm ci` fails: `Invalid: lock file's typescript@5.8.3 does not satisfy typescript@7.0.2` (and missing optional platform deps `@typescript/typescript-*@7.0.2`). | Run `npm install` to regenerate the lockfile, then fix any TS7 type/lint errors that surface. |
